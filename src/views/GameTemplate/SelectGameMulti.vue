<template>
  <div class="Outter">
    ...
    <div
      class="Container"
      :class="{ 'single-column': GameData.SlotComponent == undefined }"
    >
      <div
        class="ComponentArea Rect"
        v-if="GameData.SlotComponent != undefined"
      >
        <component
          :ID="this.id"
      ...
      <div class="Questions Rect">
        <div v-for="(Question, index) in GameData.Questions" class="Question">
          <div
            v-for="(item, partIndex) in splitQuestion(Question.Question)"
            class="part-container"
          >
            <p v-if="!isPlaceHolder(item)">{{ item }}</p>
            <div
              v-else
              v-for="(Selection, item) in Question.Selection"
              @click="SelectItem(index, item)"
              class="select-component"
              :class="{
                'selected-component': this.SelectionRecord[index] == item,
              }"
            >
              <component
                :ID="this.id"
                :is="this.GameData.SelectionType"
                :Data="Selection"
              ></component>
            </div>
          </div>
